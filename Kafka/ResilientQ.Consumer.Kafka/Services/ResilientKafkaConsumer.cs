...
public class ResilientKafkaConsumer<TMessage> : IResilientKafkaConsumer<TMessage> where TMessage : class
{
    ...
    public Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        // Build list of topics with their consumer counts and delays
        var topicConfigs = new List<(string TopicName, int ConsumerCount, string GroupId, TimeSpan? Delay)>();

        // Add main topic
        topicConfigs.Add((_configuration.TopicName, _configuration.ConsumerNumber, _configuration.GroupId, null));

        // Add retry topics
        foreach (var retryTopic in _configuration.RetryPolicy.RetryTopics)
        {
            topicConfigs.Add((
                retryTopic.TopicName, 
                retryTopic.ConsumerNumber,
                retryTopic.GroupId ?? _configuration.GroupId,
                retryTopic.GetDelayTimeSpan()
                ));
        }

        _logger.LogInformation(
            "Starting consumers - Main: {MainTopic} ({MainCount} consumers), Retry topics: {RetryTopics}",
            _configuration.TopicName,
            _configuration.ConsumerNumber,
            string.Join(", ", _configuration.RetryPolicy.RetryTopics.Select(rt =>
                $"{rt.TopicName} ({rt.ConsumerNumber} consumers, delay: {rt.Delay})")));

        // Start consumers for each topic
        var globalConsumerId = 0;
        foreach (var (topicName, consumerCount, groupId, delay) in topicConfigs)
        {
            for (int i = 0; i < consumerCount; i++)
            {
                var consumerId = globalConsumerId++;
                var task = Task.Run(() => ConsumeAsync(consumerId, topicName, groupId, delay, linkedCts.Token), linkedCts.Token);
                _consumerTasks.Add(task);
            }
        }

        _logger.LogInformation("Total consumers started: {TotalCount}", globalConsumerId);
    ...
    private async Task ConsumeAsync(int consumerId, string topic, string groupId, TimeSpan? topicDelay, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Consumer {ConsumerId} started, subscribed to topic: {Topic}{DelayInfo}",
                consumerId,
                topic,
                topicDelay.HasValue ? $" (delay: {topicDelay.Value})" : "");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            // Check if message needs to be delayed before processing
            await DelayIfNeeded(consumeResult, cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            // TODO: Comeback to decide what to do in this case
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (retryMetadata.RetryTopicIndex == -1)
        {
            ...
            await SendToRetryTopicAsync(producer, consumeResult, firstRetryTopicConfig.TopicName, newMetadata, firstRetryTopicConfig.GetDelayTimeSpan(), cancellationToken);
            return;
        }
        ...
        if (attemptsInCurrentTopic < maxAttemptsForCurrentTopic)
        {
            ...
            await SendToRetryTopicAsync(producer, consumeResult, currentRetryTopicConfig.TopicName, newMetadata, currentRetryTopicConfig.GetDelayTimeSpan(), cancellationToken);
        }
        else if (retryMetadata.RetryTopicIndex + 1 < retryTopics.Count)
        {
            ...
            await SendToRetryTopicAsync(producer, consumeResult, nextRetryTopicConfig.TopicName, newMetadata, nextRetryTopicConfig.GetDelayTimeSpan(), cancellationToken);
        }
        else
        {
    ...
    private async Task DelayIfNeeded(ConsumeResult<string, byte[]> consumeResult, CancellationToken cancellationToken)
    {
